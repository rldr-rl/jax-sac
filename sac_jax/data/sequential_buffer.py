...
if __name__ == '__main__':
    num_envs = 8
    dummy_transition = {
        'obs': np.zeros((num_envs, 24)),  # 예: 24차원 관측
        'action': np.zeros((num_envs, 4)),  # 예: 4차원 행동
        'reward': np.zeros((num_envs,)),
        'done': np.zeros((num_envs,)),
        'next_obs': np.zeros((num_envs, 24)),
    }

    buffer = SequentialReplayBuffer(
        capacity=100000,
        dummy_input=dummy_transition,
        num_envs=num_envs,
        seed=42,
    )

    for step in range(200):  # 200 step 동안 수집
        transition = {
            'obs': np.random.randn(num_envs, 24),
            'action': np.random.randn(num_envs, 4),
            'reward': np.random.randn(num_envs,),
            'done': np.random.randint(0, 2, size=(num_envs,)),
            'next_obs': np.random.randn(num_envs, 24),
        }
        buffer.insert(transition)  # 8개 env 전체 삽입
